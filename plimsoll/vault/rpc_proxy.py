"""
Python SDK bridge for the Plimsoll V4 RPC Proxy.

Instead of connecting directly to Alchemy/Infura, the agent points its
web3 provider at the Plimsoll RPC Proxy. This module makes it seamless:
...
class RPCProxyConfig:
    """Configuration for the Plimsoll RPC Proxy client."""
...
@dataclass
class PlimsollRPCProxy:
    """Python client for the Plimsoll V4 RPC Proxy.

    Usage::

        from plimsoll.vault import PlimsollRPCProxy, RPCProxyConfig
        ...
        # Option A: Get a Web3 instance that routes through Plimsoll
        w3 = proxy.get_web3()
    ...
    def get_web3(self) -> Any:
        """Return a Web3 instance connected to the Plimsoll RPC Proxy.
        ...
        logger.info("Web3 connected to Plimsoll RPC Proxy at %s", self.config.proxy_url)
        return w3

    def send_rpc(self, method: str, params: list[Any]) -> Any:
        """Send a raw JSON-RPC request to the Plimsoll proxy.
        ...
        if "error" in data and data["error"] is not None:
            ...
            raise RuntimeError(f"Plimsoll RPC error: {msg}")

        return data.get("result")

    def health_check(self) -> bool:
        """Check if the Plimsoll RPC Proxy is reachable."""
        import requests
        try:
            resp = requests.get(
