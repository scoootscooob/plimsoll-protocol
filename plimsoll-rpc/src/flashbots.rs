...
//!
//! When enabled, transactions that pass Plimsoll physics checks are routed
...
//!
//! ```text
//! Agent tx → Plimsoll Simulation → [PASS] → Flashbots Bundle
//!                                           ├── User Transaction
...
pub fn validate_bundle_deadline(bundle: &FlashbotsBundle, max_deadline_secs: u64) -> Result<()> {
    ...
    let max_ts = bundle.max_timestamp.ok_or_else(|| {
        anyhow::anyhow!(
            "PLIMSOLL ZERO-DAY 3: Bundle missing max_timestamp — open-ended intents \
    ...
    if max_ts > now + deadline {
        anyhow::bail!(
            "PLIMSOLL ZERO-DAY 3: Bundle deadline too far ({} secs > {} secs max). \
    ...
    if max_ts < now {
        anyhow::bail!(
            "PLIMSOLL ZERO-DAY 3: Bundle deadline already expired (max_ts={}, now={})",
            max_ts, now,
        );
    }
