"""
@with_aegis_firewall — Drop-in decorator for agent action functions.

Usage::

    from aegis import with_aegis_firewall, AegisFirewall

    firewall = AegisFirewall()

    @with_aegis_firewall(firewall)
    def send_transaction(payload: dict) -> dict:
        # This only runs if Aegis allows it
...
def with_aegis_firewall(
    firewall: AegisFirewall,
    *,
    spend_key: str = "amount",
    on_block: Callable[[Verdict], Any] | None = None,
) -> Callable:
    """Decorator factory that wraps an agent action with the Aegis firewall.

    Args:
        firewall: The AegisFirewall instance to evaluate against.
    ...
    def decorator(fn: Callable) -> Callable:
        ...
        def wrapper(payload: dict[str, Any], *args: Any, **kwargs: Any) -> Any:
            ...
            if verdict.blocked:
                logger.warning(
                    "[AEGIS] Blocked call to %s: %s", fn.__name__, verdict.reason
                )
