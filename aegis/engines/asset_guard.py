...
# When a transaction targets one of these, Aegis must unroll the calldata
# to extract the L2/cross-chain destination address. If the destination
# is not an Aegis-secured vault, the transaction is hard-blocked.
...
class AssetGuardEngine:
    ...
    def _check_bridge_destination(self, payload: dict[str, Any]) -> Verdict | None:
        ...
        sets the L2 destination to a hacker's address. Aegis was "spatially blind"
        ...
        if dest_lower not in approved_lower:
            return Verdict(
                ...
                reason=(
                    ...
                    f"Aegis-secured vault. L1 target {target} is a bridge — "
