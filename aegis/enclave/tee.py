...
In V1/V2, Aegis's KeyVault runs in the same OS process as the agent.
A sophisticated attacker who roots the OS can ``kill -9`` the Aegis
...
``sign()``, and ``get_public_key()`` for your hardware, and Aegis
routes all signing through it.

When no hardware TEE is available, Aegis falls back to the software
...
Security model:
    ...
    2. TEE runs Aegis's engine chain INSIDE the enclave.
...
class TEEEnclave:
    ...
    Usage::
        ...
        tee = TEEEnclave(config=TEEConfig(
            enabled=True,
            backend=NitroBackend(enclave_id="aegis-prod"),
        ))
