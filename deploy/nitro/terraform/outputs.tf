...
output "kms_key_alias" {
  ...
  value       = var.create_kms_key ? aws_kms_alias.aegis_enclave[0].name : "N/A (external key)"
}
...
output "iam_role_arn" {
  ...
  value       = aws_iam_role.aegis_enclave.arn
}
...
output "bootstrap_instructions" {
  ...
  value       = <<-INSTRUCTIONS
    1. Build the enclave image:
       docker build -t aegis-enclave:latest -f deploy/nitro/Dockerfile.enclave .
       nitro-cli build-enclave --docker-uri aegis-enclave:latest --output-file aegis.eif
    ...
    3. Copy the EIF to the instance:
       scp aegis.eif ec2-user@<instance_ip>:/opt/aegis/aegis.eif
