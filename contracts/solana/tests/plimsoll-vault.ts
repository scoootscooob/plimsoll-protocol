/**
 * Plimsoll Vault — Solana Anchor Program Tests.
 *
 ...
 * BOTH the agent AND the Plimsoll proxy cosigner sign the instruction.
 */

import * as anchor from "@coral-xyz/anchor";
import { Program } from "@coral-xyz/anchor";
import { PlimsollVault } from "../target/types/plimsoll_vault";
import { assert } from "chai";

describe("plimsoll-vault", () => {
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);

  const program = anchor.workspace.PlimsollVault as Program<PlimsollVault>;
  const owner = provider.wallet;

  // Cosigner (simulates the Plimsoll Rust Proxy Ed25519 key)
  ...
  before(async () => {
    ...
    [vaultPda, vaultBump] = anchor.web3.PublicKey.findProgramAddressSync(
      [Buffer.from("plimsoll-vault"), owner.publicKey.toBuffer()],
      program.programId
    );

    [sessionPda, sessionBump] = anchor.web3.PublicKey.findProgramAddressSync(
      [
        Buffer.from("plimsoll-session"),
        vaultPda.toBuffer(),
