...
//! `[b"aegis-vault", owner.key()]`.  The agent cannot move lamports
//! without presenting a valid proxy co-signature.
//!
//! ## Security Model
//!
//! ```text
//!   Agent → Aegis Rust Proxy (7 engines) → Ed25519 sign → CPI → PDA vault
//! ```
//!
...
pub const VAULT_SEED: &[u8] = b"aegis-vault";
/// Seed prefix for session key accounts.
pub const SESSION_SEED: &[u8] = b"aegis-session";
...
pub mod aegis_vault {
    ...
    /// Initialize a new Aegis Vault PDA for the owner.
    pub fn initialize(
        ...
        msg!("Aegis Vault initialized for owner {}", vault.owner);
        Ok(())
    }
    ...
    pub fn withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
        ...
        require!(vault_lamports >= amount, AegisError::InsufficientBalance);
    ...
    ///
    /// Requires BOTH the agent signature AND the Aegis proxy cosignature.
    ...
    pub fn execute(ctx: Context<Execute>, amount: u64) -> Result<()> {
        ...
        // ── 1. Emergency lock ────────────────────────────────────
        require!(!vault.emergency_locked, AegisError::EmergencyLocked);

        // ── 2. Session validity ──────────────────────────────────
        require!(session.active, AegisError::SessionInactive);
        require!(now < session.expires_at, AegisError::SessionExpired);

        // ── 3. Single-tx cap ─────────────────────────────────────
        require!(
            amount <= session.max_single_amount,
            AegisError::ExceedsSingleTxCap
        );
        ...
        require!(
            session.spent_today.checked_add(amount).ok_or(AegisError::ArithmeticOverflow)?
                <= session.daily_budget,
            AegisError::ExceedsDailyBudget
        );
        ...
        if vault.velocity_max_per_hour > 0 {
            ...
            require!(
                ...
                AegisError::VelocityLimitExceeded
            );
        ...
        if vault.max_drawdown_bps > 0 && vault.initial_balance > 0 {
            ...
            let floor = vault
                ...
                .ok_or(AegisError::ArithmeticOverflow)?
                / 10_000;
            require!(balance_after >= floor, AegisError::DrawdownFloorBreached);
        }
    ...
    /// Update the Aegis proxy cosigner pubkey (owner only).
...
///
/// Seeds: `["aegis-vault", owner_pubkey]`
#[account]
pub struct VaultState {
    /// Owner pubkey (human / DAO).
    pub owner: Pubkey,
    /// Aegis Rust Proxy cosigner pubkey — every agent tx must carry this sig.
    pub cosigner: Pubkey,
    /// Emergency lock flag.
...
///
/// Seeds: `["aegis-session", vault_pubkey, agent_pubkey]`
#[account]
...
pub struct Execute<'info> {
    ...
    /// The Aegis proxy cosigner — MUST sign to prove off-chain engines passed.
    #[account(
        constraint = cosigner.key() == vault.cosigner @ AegisError::InvalidCosigner,
    )]
