...
/**
 * @title PlimsollEASAdapter — Bridge between PlimsollAttestation and Ethereum Attestation Service
 *
 * @notice Translates Plimsoll PoBR attestations into EAS-compatible attestations.
 *         This allows DeFi protocols that already integrate with EAS to verify
 *         Plimsoll-governed vault risk parameters without custom integrations.
 *
 ...
 *
 * Architecture:
 *   PlimsollVault → PlimsollAttestation (on-chain registry)
 *                       ↓
 *               PlimsollEASAdapter (this contract)
 *                       ↓
...
/// @dev Minimal interface for the PlimsollAttestation registry.
interface IPlimsollAttestation {
    struct Attestation {
        address vault;
        address owner;
...
contract PlimsollEASAdapter {
    // ── State ───────────────────────────────────────────────────

    /// @notice The Plimsoll PoBR attestation registry.
    IPlimsollAttestation public immutable plimsollRegistry;

    /// @notice The Ethereum Attestation Service contract.
    IEAS public immutable eas;

    /// @notice The EAS schema UID for Plimsoll PoBR attestations.
    ...
    constructor(
        address plimsollRegistry_,
        address eas_,
        bytes32 schemaUID_
    ) {
        require(plimsollRegistry_ != address(0), "EASAdapter: zero registry");
        require(eas_ != address(0), "EASAdapter: zero EAS");

        plimsollRegistry = IPlimsollAttestation(plimsollRegistry_);
        eas = IEAS(eas_);
    ...
    /**
     * @notice Create an EAS attestation for an Plimsoll-governed vault.
     * @dev Reads the vault's PoBR data from PlimsollAttestation and encodes
    ...
    function createEASAttestation(address vault) external returns (bytes32 uid) {
        require(plimsollRegistry.isAttested(vault), "EASAdapter: vault not attested in Plimsoll");
        require(easAttestations[vault] == bytes32(0), "EASAdapter: already attested in EAS");

        IPlimsollAttestation.Attestation memory a = plimsollRegistry.getAttestation(vault);
     ...
     * @dev Should be called when the Plimsoll attestation is revoked
    ...
    /**
     * @notice Sync an existing Plimsoll attestation update to EAS.
    ...
    /**
     * @notice Compute the EAS schema UID for the Plimsoll PoBR schema.
