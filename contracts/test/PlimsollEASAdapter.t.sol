...
import "../src/PlimsollEASAdapter.sol";
...
/// @title PlimsollEASAdapter Test Suite
contract PlimsollEASAdapterTest is Test {
    PlimsollAttestation registry;
    MockEAS mockEAS;
    PlimsollEASAdapter adapter;

    address vault1 = address(0xABC);
    address vaultOwner = address(0xA11CE);
    bytes32 schemaUID = keccak256("plimsoll-pobr-v1");

    function setUp() public {
        registry = new PlimsollAttestation();
        mockEAS = new MockEAS();
        adapter = new PlimsollEASAdapter(
            address(registry),
            address(mockEAS),
            schemaUID
        );

        // Create an attestation in the Plimsoll registry
        registry.attest(
            vault1,
            vaultOwner,
            500,            // 5% max drawdown
    ...
    function test_constructor_sets_addresses() public view {
        assertEq(address(adapter.plimsollRegistry()), address(registry));
    ...
    function test_constructor_rejects_zero_registry() public {
        ...
        new PlimsollEASAdapter(address(0), address(mockEAS), schemaUID);
    }

    function test_constructor_rejects_zero_eas() public {
        vm.expectRevert("EASAdapter: zero EAS");
        new PlimsollEASAdapter(address(registry), address(0), schemaUID);
    }
    ...
    function test_create_eas_attestation_not_attested() public {
        ...
        vm.expectRevert("EASAdapter: vault not attested in Plimsoll");
    ...
    function test_create_emits_event() public {
        ...
        emit PlimsollEASAdapter.EASAttestationCreated(vault1, bytes32(uint256(1)));
        adapter.createEASAttestation(vault1);
    }
    ...
    function test_update_schema() public {
        bytes32 newSchema = keccak256("plimsoll-pobr-v2");
